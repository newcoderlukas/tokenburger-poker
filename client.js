...
// IMPORTANT: use sessionStorage (per-tab) — NOT localStorage — so that two tabs
// in the same browser don't collide. localStorage is shared between all tabs of
// the same origin, which caused a new joiner in a 2nd tab to "reconnect" as the
// player from the 1st tab instead of being added as a fresh player.
// sessionStorage survives a page reload (F5) but is unique per tab, which is
// exactly what we want for reconnect after a dropped connection.
let persistentId = sessionStorage.getItem('tk_pid');
if (!persistentId) {
  persistentId = 'p_' + Math.random().toString(36).slice(2, 10) + Date.now().toString(36);
  sessionStorage.setItem('tk_pid', persistentId);
}
// One-time cleanup of the old shared pid so stale localStorage doesn't haunt us.
try { localStorage.removeItem('tk_pid'); } catch (_) {}

const socket = io({ auth: { persistentId } });

// ------------------------------------------------------------
// Viewport height fix for mobile (iOS Safari, Android Chrome)
// ------------------------------------------------------------
// Some phones have dynamic browser chrome (bottom address bar, gesture bar, etc)
// that `100vh` and even `100dvh` don't measure correctly in every version.
// We track the *actual* usable height via visualViewport / innerHeight and
// expose it as --app-h, which our CSS consumes as the authoritative height.
function updateAppHeight() {
  // visualViewport gives the ACTUAL visible area excluding keyboards, browser
  // UI overlays, etc. Fall back to innerHeight where it isn't supported.
...
$('btn-join').addEventListener('click', () => {
  ...
  socket.emit('joinRoom', { name, code, persistentId }, (res) => {
    ...
    myRoom = res.code;
    if (res.assignedId && res.assignedId !== persistentId) {
      // Server detected a pid collision (or we reconnected) and gave us a
      // definitive id. Save it so future reconnects resolve to the right player.
      persistentId = res.assignedId;
      sessionStorage.setItem('tk_pid', persistentId);
    }
    localStorage.setItem('tk_room', res.code);
